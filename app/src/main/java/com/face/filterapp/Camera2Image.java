...
public class Camera2Image extends AppCompatActivity {
    ...
    private void showToast(final String text) {
        ...
        if (this != null) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(Camera2Image.this, text, Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mFile = new File(Environment
                .getExternalStorageDirectory()+IMAGE_DIRECTORY, "/img_"+ String.valueOf(System.currentTimeMillis())+".jpg");
        addVideo(mFile,"img_"+ String.valueOf(System.currentTimeMillis()));
    }
            ...
            for (String cameraId : manager.getCameraIdList()) {
                ...
                if (facing == CameraCharacteristics.LENS_FACING_FRONT) {
                    ...
                    if (mOverlay != null) {
                        ...
                        int min = Math.min(mPreviewSize.getWidth(), mPreviewSize.getHeight());
                        int max = Math.max(mPreviewSize.getWidth(), mPreviewSize.getHeight());
                        mOverlay.setCameraInfo(min, max, facing);
    ...
    /**
     * Opens the camera specified by {@link Camera2Image#mCameraId}.
     */
...
//                                        CameraMetadata.STATISTICS_FACE_DETECT_MODE_FULL);
    ...
    public void onRequestPermissionsResult(int requestCode,
        ...
        if (requestCode == REQUEST_CAMERA_PERMISSION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                if (ActivityCompat.checkSelfPermission(Camera2Image.this, Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) {
                    if (hasStoragePermission()) {
                       // createCameraSource();
                    }
                }
            } else {
                Toast.makeText(Camera2Image.this, "Permission denied!", Toast.LENGTH_SHORT).show();
    ...
    private boolean allPermissionGranted() {
        ...
        if (ActivityCompat.checkSelfPermission(Camera2Image.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(Camera2Image.this, new String[]{Manifest.permission.CAMERA}, REQUEST_CAMERA_PERMISSION);
            return true;
        } else {
